...
public extension CameraManager {
    ...
    var professionalISO: Float { attributes.cameraExposure.iso }
    var professionalExposureDuration: CMTime { attributes.cameraExposure.duration }
    var professionalExposureBias: Float { attributes.cameraExposure.targetBias }
     ...
     var professionalEquivalentFocalLength: Int {
         let baseFocalLength: CGFloat
         switch activeLens {
         case .ultraWide: baseFocalLength = 13
         case .wide: baseFocalLength = 24
         case .telephoto: baseFocalLength = 77
         }
         return Int((baseFocalLength * max(1, attributes.zoomFactor)).rounded())
     }
    ...
    func setProfessionalFlash(_ mode: CameraFlashMode) {
        try? changeFlashMode(mode)
    }

    func setProfessionalTorch(_ mode: CameraTorchMode) {
        try? changeTorchMode(mode)
    }

    func setProfessionalExposureMode(_ mode: AVCaptureDevice.ExposureMode) {
        try? changeExposureMode(mode)
    }

    func setProfessionalISO(_ value: Float) {
        try? changeISO(value)
    }

    func setProfessionalExposureDuration(_ value: CMTime) {
        try? changeExposureDuration(value)
    }

    func setProfessionalExposureBias(_ value: Float) {
        try? changeExposureTargetBias(value)
    }
